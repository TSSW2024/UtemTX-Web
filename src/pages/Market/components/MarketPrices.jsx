...
const MarketPrices = ({ cryptoData, currentCategory, handleClick, handleListClick }) => {
    ...
    return (
        <div className="text-base flex flex-col gap-4 px-4 py-2">
            <h1 className="text-xl sm:text-2xl font-bold">Precios del mercado Cripto</h1>
            <div className="flex flex-wrap text-base gap-4">
                {Object.keys(cryptoData).map((category) => (
                    <div
                        key={category}
                        className={`rounded-lg px-2 sm:px-4 py-2 w-full sm:w-1/3 cursor-pointer rounded-2xl shadow-lg dark:text-gray-200 drop-shadow hover:bg-zinc-300 dark:hover:bg-gray-700 ${currentCategory === category ? 'bg-zinc-300 dark:bg-gray-700' : 'bg-zinc-200 dark:bg-gray-800'
                            }`}
            ...
            {cryptoData[currentCategory] && cryptoData[currentCategory].length > 0 ? (
                <div className="rounded-lg overflow-hidden bg-white dark:bg-gray-800">
                    <div className="flex bg-gray-100 dark:bg-gray-700">
                        <div className="w-2/12 sm:w-1/12 p-2 text-center font-semibold">Nº</div>
                        <div className="w-5/12 sm:w-4/12 p-2 text-center font-semibold">Nombre</div>
                        <div className="w-3/12 sm:w-3/12 p-2 text-center font-semibold">Precio</div>
                        <div className="w-2/12 sm:w-4/12 p-2 text-center font-semibold">24h Change</div>
                    </div>
                    <div className="h-full overflow-y-auto">
                        {cryptoData[currentCategory].map((item, index) => (
                            ...
                            >
                                <div className="w-2/12 sm:w-1/12 p-2 text-center">{index + 1}</div>
                                <div className="w-5/12 sm:w-4/12 p-2 flex items-center gap-4">
                                    <img
                                        src={item.image}
                                        alt={item.name}
                                ...
                                <div className="w-3/12 sm:w-3/12 p-2 text-center">{item.price}</div>
                                <div className={`w-2/12 sm:w-4/12 p-2 text-center ${getChangeColor(item.change_24h)}`}>
                                    {item.change_24h}
                                </div>
                            </div>
                        ))}
